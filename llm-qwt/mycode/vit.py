...
def main():
    ...
    if for_fast_test:
        subset_indices = list(range(100))  # 截取前100个样本
        subset = Subset(loader_eval.dataset, subset_indices)
        subset_loader = DataLoader(
            subset,
            batch_size=loader_eval.batch_size,
            shuffle=False,
            num_workers=loader_eval.num_workers
        )
        loader_eval = subset_loader
    '''
    ...
    if for_fast_test:
        _write('Fast test mode, only using 3 samples for calibration')
    ...
    _write('quant eval_acc: {:.2f}'.format(top1_acc_eval.avg))
    with open(f'log/{args.model}/acc.log', 'a') as f:
        f.writelines(f'quant: {top1_acc_eval.avg}\n') 
    # exit()
    '''
    ...
    with open(f'log/{args.model}/acc.log', 'a') as f:
        f.writelines(f'qwt: {top1_acc_eval.avg}\n') 
    '''
...
def validate(model, loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(tqdm((loader), total = len(loader))):
