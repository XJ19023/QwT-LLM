...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.is_causal = True
        self.q_proj = quantLinear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=True)
        self.k_proj = quantLinear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj = quantLinear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.o_proj = quantLinear(config.num_attention_heads * self.head_dim, config.hidden_size, bias=False)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward_before_blocks(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        with open('log/123.log', 'a') as f:
            f.writelines(f'>>>> Forward <<<<\n')
            f.writelines(f'input_ids type: {type(input_ids)} {input_ids.shape}\n')
            f.writelines(f'attention_mask type: {attention_mask}\n')
            ...
            f.writelines(f'output_hidden_states type: {type(flash_attn_kwargs)} {flash_attn_kwargs}\n')
            f.writelines(f'cache_position type: {type(cache_position)} {cache_position}\n\n')
            f.writelines(f'flash_attn_kwargs type: {type(flash_attn_kwargs)} {flash_attn_kwargs}\n\n')
        '''
        ...
        return hidden_states, position_ids, cache_position, position_embeddings
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # with open('log/123.log', 'a') as f:
        #     f.writelines(f'>>>> Forward <<<<\n')
        #     f.writelines(f'layerID type: {type(layerID)}\n\n')
        #     f.writelines(f'input_ids type: {type(input_ids)} {input_ids.shape}\n')
        #     f.writelines(f'attention_mask type: {attention_mask}\n')
        #     f.writelines(f'position_ids type: {type(position_ids)} {position_ids}\n')
        #     f.writelines(f'past_key_values type: {type(past_key_values)} {past_key_values}\n')
        ...
        for layerID, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
            
            # append_activation(f'layers.{layerID}.input', hidden_states)
