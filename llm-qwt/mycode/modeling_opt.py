...
def lienar_regression(X, Y, block_id=0):
    ...
    X = gather_tensor_from_multi_processes(X, world_size=1)

    X_add_one = torch.cat([X, torch.ones(size=[X.size(0), ], device=X.device).reshape(-1, 1)], dim=-1)
    Y = Y.reshape(-1, Y.size(-1))

    Y = gather_tensor_from_multi_processes(Y, world_size=1)
...
class OPTAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.k_proj = quantLinear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        self.v_proj = quantLinear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        self.q_proj = quantLinear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        self.out_proj = quantLinear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.fc1 = quantLinear(self.embed_dim, config.ffn_dim, bias=config.enable_bias)
        self.fc2 = quantLinear(config.ffn_dim, self.embed_dim, bias=config.enable_bias)
    ...
    def forward(
        ...
        """
        # print(f"hidden_states: {hidden_states.shape}, attention_mask: {attention_mask.shape if attention_mask is not None else None}, layer_head_mask: {layer_head_mask.shape if layer_head_mask is not None else None}, past_key_value: {past_key_value}, output_attentions: {output_attentions}, use_cache: {use_cache}")
        # if layer_idx == 1:
        #     with open('log/123.log', 'a') as f:
        #         f.writelines(f' layerID {layer_idx} In\n')
        #         f.writelines(f' hidden_states: {hidden_states.view(-1)[:5]} \n')
        #         f.writelines(f' attention_mask: {attention_mask} \n')
        #         f.writelines(f' layer_head_mask: {layer_head_mask} \n')
        #         f.writelines(f' past_key_value: {past_key_value} \n')
        #         f.writelines(f' output_attentions: {output_attentions} \n')
        #         f.writelines(f' use_cache: {use_cache} \n')
        ...
        # if layer_idx == 0:
        #     with open('log/123.log', 'a') as f:
        #         f.writelines(f' layerID {layer_idx}  Out\n')
        #         f.writelines(f' hidden_states: {hidden_states.view(-1)[:5]} \n')
        #         f.writelines(f' attention_mask: {attention_mask} \n')
        #         f.writelines(f' layer_head_mask: {layer_head_mask} \n')
        #         f.writelines(f' past_key_value: {past_key_value} \n')
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def forward_before_blocks(
        ...
        return hidden_states, causal_attention_mask, use_cache
    ...
    def forward(
        ...
        # with open('log/123.log', 'a') as f:
        #     f.writelines(f'attention_mask, {attention_mask}\n')
        #     f.writelines(f'past_key_values, {past_key_values}\n')
        #     f.writelines(f'inputs_embeds, {inputs_embeds}\n')
        #     f.writelines(f'use_cache, {use_cache}\n')
        #     f.writelines(f'output_attentions, {output_attentions}\n')
        #     f.writelines(f'output_hidden_states, {output_hidden_states}\n')
        #     f.writelines(f'return_dict, {return_dict}\n')
        r"""
        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
                Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if get_save_tensor_enable():
                    ...
                    append_activation(f'layers.{idx}.attention_mask', causal_attention_mask)
                with open('log/123.log', 'a') as f:
                    f.writelines(f'layers.{idx}.input.dtype, {hidden_states.dtype}\t {hidden_states.shape}\n')
                    f.writelines(f'layers.{idx}.layer_head_mask, {head_mask[idx] if head_mask is not None else None}\n')
                    f.writelines(f'layers.{idx}.past_key_value, {past_key_value}\n')
                    f.writelines(f'layers.{idx}.output_attentions, {output_attentions}\n')
                    f.writelines(f'layers.{idx}.use_cache, {use_cache}\n')
                '''
...
class OPTForCausalLM(OPTPreTrainedModel):
    ...
    def forward(
        ...
        # print(f'input_ids: {input_ids.shape}, attention_mask: {attention_mask}, head_mask: {head_mask}, past_key_values: {past_key_values}, inputs_embeds: {inputs_embeds}, use_cache: {use_cache}, output_attentions: {output_attentions}, output_hidden_states: {output_hidden_states}, return_dict: {return_dict}')
